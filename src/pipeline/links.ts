...
export function referenceEdges(
  ...
  const matchesFor = (target: string, from: ReferenceDocInput): ReferenceDocInput[] => {
    ...
    if (target.startsWith(WIKILINK_PREFIX)) {
      ...
      // Path-style wikilinks after folder ingest: stored paths carry the
      // dropped folder as their first segment (`MyVault/projects/alpha.md`).
      // Try each corpus root; a hit under more than one root is ambiguous.
      const wikiPathKey = posixNormalize(wiki).replace(/^\/+/, '').toLowerCase();
      if (wikiPathKey.includes('/') && firstPathSegments.size > 0) {
        ...
        for (const root of firstPathSegments) {
          takeRoot(byPath.get(`${root}/${wikiPathKey}`));
          if (!posixBasename(wikiPathKey).includes('.')) {
            takeRoot(byPath.get(`${root}/${wikiPathKey}.md`));
          }
        }
        if (rootHits.length === 1) take(rootHits);
        if (found.length > 0) return found;
      }
      if (pathQualified) return found;
