 ...
 * onmessage switch over these so the message contract can be unit-tested
...
export function handleLexical(
  req: Extract<AggRequest, { type: 'lexical' }>,
): Extract<AggResponse, { type: 'lexical:done' }> {
...
export async function handleSemantic(
  req: Extract<AggRequest, { type: 'semantic' }>,
  onProgress?: (msg: Extract<AggResponse, { type: 'semantic:progress' }>) => void,
): Promise<Extract<AggResponse, { type: 'semantic:done' }>> {
...
export function handleCluster(
  req: Extract<AggRequest, { type: 'cluster' }>,
): Extract<AggResponse, { type: 'cluster:done' }> {
  return {
    requestId: req.requestId,
    type: 'cluster:done',
    clusters: clusterFromEdges(req.ids, req.edges),
  };
}
...
export async function dispatchAggregatorRequest(
  ...
  try {
    ...
    } else post(handleCluster(req));
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    post({ requestId: req.requestId, type: 'error', message });
  }
}
