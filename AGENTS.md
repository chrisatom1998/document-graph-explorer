...
- **Hello-world / smoke test**: on the welcome screen click **"Load demo corpus"**, then wait (Parsing → Embedding → Clustering) for the force-directed 3D graph of exactly **100 document nodes** (36 committed + 64 generated PDFs) to render — ~20-60s on a fast machine, a few minutes on shared or software-rendered environments. First run downloads/loads the bundled embedding model, so the initial ingest is slow.
