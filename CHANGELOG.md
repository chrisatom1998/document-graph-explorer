...
### Added
- Gemini 3.7 Flash (`google/gemini-3.7-flash`) in the OpenRouter chat and enrichment model pickers. Chat lists it with the other fast-tier options; enrichment lists it next to Gemini 3.1 Flash Lite so corpus runs can use the latest Flash without leaving the cheap tier.
- Additional curated OpenRouter models that fill provider gaps: enrichment adds GPT-5.6 Luna, DeepSeek V4 Flash, and MiniMax M3; chat adds GPT-5.6 Sol, Grok 4.6, DeepSeek V4 Pro, and Luna. Defaults stay Claude Sonnet 5 (chat) and Gemini 3.1 Flash Lite (enrichment).
