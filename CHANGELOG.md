...
### Added
- **Add a folder** (Google Drive-style folder ingest): a first-class picker on the welcome screen and toolbar that selects a directory once and ingests every relevant file inside it, subfolders included — no drilling into subfolders or drag-and-drop required. Uses `window.showDirectoryPicker()` where available, falling back to a `webkitdirectory` folder-upload picker elsewhere; both paths run the same relevance filters as a folder drop (`.gitignore`, ignored dirs, lockfiles, dotfiles, size caps, ignored-tray routing). One-shot import — live syncing stays with the corpus switcher's "Watch a folder".
...
### Fixed
- Shareable graph links now open when the app is already running (PWA / same-tab hash changes, plus `launch_handler` so an installed app navigates to the new URL instead of only focusing), survive messenger encodings (`%23graph=`, `#graph%3D`) and chat apps that strip `#` entirely (compact links include a `?graph=` copy), and rewrite localhost/file/Electron origins to the public web app so recipients can actually open them. The share dialog shows the URL and copies it after the link is built, so mobile clipboard permissions keep the user gesture. Shared/imported documents show their included summary instead of "text unavailable".
